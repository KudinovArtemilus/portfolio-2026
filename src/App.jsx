...
### ⚠️ Аварийная сигнализация (Alarming)
...
### 📂 Мониторинг конфигурационных файлов (Recipes)
...
- 🚀 Масштабируемость: Благодаря RabbitMQ система выдержит сотни одновременно подключенных операторов.
- ⚡ Производительность: Использование TimescaleDB гарантирует, что графики будут летать даже через год эксплуатации.
- 🛡️ Безопасность: Ролевая модель (RBAC) на базе Spring Security с шифрованием паролей BCrypt.
- 🇷🇺 Локализация: Полная поддержка кириллицы и специфических кодировок (Windows-1251) для совместимости со старым оборудованием.

---

## 5. Слабые стороны и точки риска (Weaknesses)

- 🛑 Отказоустойчивость бэкенда: Текущая архитектура — одиночный сервер. При критической эксплуатации потребуется внедрение кластера (Docker Swarm/K8s).
- 🔒 Шифрование трафика: В базе настроен HTTP. Для внешней сети (через Интернет) необходимо развернуть SSL-сертификаты для HTTPS/WSS.
- 📲 Каналы уведомлений: Оповещения пока живут только внутри браузера. Системе "не хватает" интеграции с Telegram или Email-ботами.
- 🔌 Закрытость протоколов: На данный момент драйвер сфокусирован на S7. Для универсальности стоит добавить поддержку Modbus и OPC UA.
- 🖥️ Нагрузка на браузер: При выводе сотен графиков на один экран нагрузка на процессор клиента может быть высокой.
`;
